# Cypress Test Documenter
This plugin helps you generate and document your cypress commands in all one code base!!!, to get started download it and read the How to install section below.

### How to Install

1. open the plugin folder in any terminal of your choice or vs code and run  `npm install` this makes sure it gets the required node modules (all it use is excel js)

2. ```npm i ../<path you downloaded the file to>```  (we always recommend putting it in the same folder as your **Test folder** so it can be easy to point to when running the `npm i` command)

3. Run the command ```npx setup-test-docs``` to add imports to the necessary files and configure the snippets file.

4. Once you've successfully installed the plugin in your project follow just add the required (3) lines of code to the specified files as stated below

#### In cypress.config file add 
```
import { defineConfig } from "cypress";
import { registerTestDocumentation } from 'cypress_test_documenter/src/node' // already added

export default defineConfig({
  allowCypressEnv: false,

  e2e: {
    setupNodeEvents(on, config) {
     registerTestDocumentation(on) // add this
     return config // add this
    },
  },
});
```

#### support/e2e.ts
`import 'cypress_test_documenter/src/browser' ` // add this line

## How to use

expected data
```{
  suite: string
  /** Optional e.g. "TC-HOME-001" */
  id?: string 
  /** Optional - provided by the description of the it block */
  description?: string
  testData: string
  expectedResult: string
  /** Optional — leave blank to fall back to the sheet's "Assigned" column being empty */
  assigned?: string
  /** Optional — e.g. "Tested on the following Browsers: Brave, Chrome, and Microsoft Edge" */
  comment?: string
  /**
   * Optional override. If omitted, the afterEach hook fills this in automatically:
   */
  actualResultOverride?: string
}```

``` 
it('<expectedResult>', () => {
  // this must be specified at the top of each it block
    cy.docTest({
      suite: "Admin Page",
      id: "TC_ADM_001", // Optional field, this could be generated by the system
      testData: "admin: {email: \"email", password: \"password\"}",
      description: "<Description of Your Test>"
    })
    cy.precondition("User Signed In")

    cy.procedure("On navbar select VAIDs application")
    cy.visit('/')

    cy.procedure("Click on the 'Login' button")
   
    cy.contains('login', { matchCase: false })
      .click()

    cy.procedure("Check to make sure user is Signed In successfully)
    cy.url().should('contain', 'dashboard')
  })
```

## Best Practices



## Frequently Asked Questions (FAQ)
**Q1** - I followed all the steps but my cypress shows some errors <br>
**Ans** : make sure you are using typescript version 5.*, as cypress doesnt currently support higher versions like 7

**Q2** - I am using typescript 5 and have everything else running but still get some tsconfig error <br>
**Ans** : This is not a plugin issue you basically forgot to add tsconfig to you project. Do this by running `npx tsc --init`
